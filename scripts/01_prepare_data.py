...
def load_eeg_channel(rec_path: Path) -> tuple[np.ndarray, float]:
    """
    Lee el archivo EDF (.rec), selecciona el canal C3-A2 y re-muestrea a FS_TARGET.
    Devuelve (signal_1d, fs_original).
    Nota: se pasa como file-like para bypassear el check de extensión de MNE.
    """
    with open(rec_path, "rb") as fh:
        raw = mne.io.read_raw_edf(fh, preload=True, verbose=False)
...
def process_subject(subj_id: int) -> tuple[np.ndarray, np.ndarray, np.ndarray] | None:
    """
